...
- `connection` is a Solana connection to any Solana network (e.g. mainnet-beta, devnet, testnet), a commitment level of 'processed' is recommended
...
export type GatewayProviderProps = {
  ...
  connection: Connection; // a Solana connection to any Solana network (e.g. mainnet-beta, devnet, testnet)
  cluster?: string; // The cluster name. Defaults to mainnet-beta. Should be set if a different connection other than mainnet-beta is used
